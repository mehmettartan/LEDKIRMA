...
namespace LEDKIRMA
{
    class getCCT
    {
        ...
        public bool CaptureAndRead(int portNumber, int numFibers, out string[] ccts)
        {
            ccts = new string[numFibers]; // CCT değerlerini tutacak dizi
            ...
            if (FeasaCom_Open(portNumber, "115200") == 1)
            {
                ...
                {
                    // Her fiber için CCT değerini almak için komut gönder
                    string fiberNumber = i.ToString("D2");
                    string command = "GETCCT" + fiberNumber;
